...
export class facture_func {
  async modif(
    ...
    if (token) {
      ...
      if (result.length) {
        var temp = Deno.readTextFileSync("./facture.html");
        temp = temp.replace(/<%=title%>/g, "Créer une facture");
        ...
        if (idfacture) {
          ...
          temp = temp.replace(/<%=facturetemplate%>/g, info[0]["html"]);

        } else {
          temp = temp.replace(
            /<%=facturetemplate%>/g,
            `<div class="column quote-generator">
          <span contenteditable="true" class="editable quote-number self-end h2 namefacture"><%=namefacture%></span>
          <div>
          ...
          <div class="relative mb3">
              <table class="quote-table-left-header width-5 medium mb0">
                  <tbody class="extrainfotable">
                      <tr>
                          <td style="background-color: rgb(231, 230, 230); color: black;"><span contenteditable="true" class="editable prevent-line-break" placeholder="En-tête">Date de facture</span></td>
                          <td class="relative">
                      ...
                      <tr>
                          ...
                          <td class="relative">
                              <div class="absolute top-0 full-height flex-center" style="right: -8px;"></div> <span contenteditable="true" class="editable prevent-line-break" placeholder="Contenu"><%=datepaiement%></span></td>
                      </tr>
                  </tbody>
              </table>
        ...
        var dateStringPaiement = date.getDate() + "/" +
          (((date.getMonth() + 1) + 2 > 12 ? 1 : (date.getMonth() + 1) + 2)
              .toString().length == 1
            ? "0" +
              ((date.getMonth() + 1) + 2 > 12 ? 1 : (date.getMonth() + 1) + 2)
            : ((date.getMonth() + 1) + 2 > 12
              ? 1
              : (date.getMonth() + 1) + 2)) +
          "/" + date.getFullYear();

        temp = temp.replaceAll(/<%=date%>/g, dateString);
        temp = temp.replaceAll(/<%=datepaiement%>/g, dateStringPaiement);

        var dernfacture = await database_execute.query_select(
          "SELECT * FROM facture WHERE user_key = '" + private_key +
            "' ORDER BY id DESC",
        );
        var newnum = (dernfacture.length == 0
          ? 1
          : parseInt(dernfacture[0]["id"]) + 1);

        var nameFacture = "Facture N°" + newnum;

        temp = temp.replace(/<%=namefacture%>/g, nameFacture);
        if (result[0]["tva"] == 0) {
        ...
        if (idclient) {
          ...
          temp = temp.replace(/<%=facturetoken%>/g, idfacture ? idfacture : "");
        } else {
          temp = temp.replace(
