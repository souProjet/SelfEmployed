...
if (!!selectclientbutton) {
    selectclientbutton.addEventListener('click', function() {
        ...
        facture.style.opacity = parseInt(suggestclientblock.style.opacity) ? ".3" : "1";
...
addprestabutton.addEventListener('click', function() {
    ...
    facture.style.opacity = (prestalistblock.style.display == "none" ? ".2" : "1");
...
function generatefacture(dl) {
    var facturehtml = facture.innerHTML;
    if (dl) {
        facturehtml = facturehtml.replace(`<button class="selectclient btn el-button hidden-button el-button--text" style="color: rgb(112, 166, 216); padding-bottom: 4px;"><span>Séléctionner un client <i class="fas fa-level-down-alt" style="vertical-align: -1px;"></i></span></button>`, '');
        facturehtml = facturehtml.replace(`<div class="absolute flex-center width-5" style="bottom: -12px;"><button class="extrainfo fas fa-plus-circle h2"></button></div>`, '');
        facturehtml = facturehtml.replace(`<div class="absolute flex-center full-width" style="bottom: -12px;"><button class="addprestabutton fas fa-plus-circle h2"></button></div>`, '');
        facturehtml = facturehtml.replaceAll(`<div onclick="delextrainfo(this);" class="absolute top-0 full-height flex-center" style="right: -8px;"><button class="fas fa-minus-circle h4"></button></div>`, '');
        facturehtml = facturehtml.replaceAll(`<div onclick="deletepresta(this);" class="absolute top-0 full-height flex-center" style="right: -8px;"><button class="fas fa-minus-circle h4"></button></div>`, '');
        facturehtml = facturehtml.replaceAll('contenteditable="true"', 'contenteditable="false"');
        facturehtml = facturehtml.replaceAll('editable', '');
    }
    return facturehtml;
}

savefacture.addEventListener('click', function() {
    var date = new Date();
    ...
    $.post("http://localhost:8003/api/addfacture/", {
        ...
        idfacture: document.body.id,
        name: document.querySelector('.namefacture').innerText,
        html: generatefacture(false),
        date: dateString
    }, function(data) {
        if (data == "ok") {
...
dlpdf.addEventListener('click', function() {
    ...
    html2pdf().set(opt).from(generatefacture(true)).save(document.querySelector('.namefacture').innerText + ".pdf");
